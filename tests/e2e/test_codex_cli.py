...
"""Codex CLI adapter boundary and opt-in real-binary integration tests."""
...
@pytest.fixture(name="isolated_codex_home", autouse=True)
def isolated_codex_home_fixture(tmp_path_factory, restore_environ) -> Path:
    home = tmp_path_factory.mktemp("codex-home")
    restore_environ["CODEX_HOME"] = str(home)
    return home


def write_mock_codex(path: Path, *, log_path: Path, env_log_path: Path) -> None:
    path.write_text(
        f"""#!{sys.executable}
import json
import os
...
thread_id = args[args.index("resume") + 1] if "resume" in args else "thread-e2e"
...
for event in [
    ...
    {{
        "type": "item.completed",
        "item": {{"id": "item-1", "type": "agent_message", "text": thread_id + ":" + prompt}},
    }},
...
def fabric_config(
    ...
    *,
    ...
    if mock_cli:
        cli_path = tmp_path / "mock-codex"
        write_mock_codex(
            cli_path,
            log_path=tmp_path / "codex-args.jsonl",
            env_log_path=tmp_path / "codex-env.jsonl",
        )
        environment_env["FABRIC_TEST_CODEX_BIN"] = str(cli_path)
    if nemo_relay_command is not None:
        environment_env["FABRIC_TEST_NEMO_RELAY_COMMAND"] = str(nemo_relay_command)
    config = FabricConfig(
        metadata=MetadataConfig(name="codex-cli-e2e"),
        harness=HarnessConfig(
            ...
            settings={
                ...
                "config_overrides": {"features.web_search": False},
            },
        ),
        models={"default": ModelConfig(provider="openai", model="openai/gpt-5.4")},
...
async def test_fabric_session_resumes_and_maps_capabilities(
    ...
    async with await Fabric().start_runtime(config, base_dir=tmp_path) as runtime:
        skill_link = tmp_path / "workspace" / ".agents" / "skills" / "review"
    ...
    assert first.output["thread_id"] == second.output["thread_id"] == "thread-e2e"
    assert first.output["response"] == "thread-e2e:first"
    assert second.output["response"] == "thread-e2e:second"
    arguments = cli_arguments(tmp_path)
    assert "resume" not in arguments[0]
    assert arguments[1][-3:] == ["resume", "thread-e2e", "-"]
    for args in arguments:
        assert args[args.index("--sandbox") + 1] == "read-only"
        assert "--skip-git-repo-check" in args
        assert args[args.index("--profile") + 1].startswith("fabric-")
    profile_name = arguments[0][arguments[0].index("--profile") + 1]
    assert not (isolated_codex_home / f"{profile_name}.config.toml").exists()
    assert not skill_link.exists()


async def test_relay_routes_mock_codex_cli_through_gateway(tmp_path):
    ...
    arguments = cli_arguments(tmp_path)
    assert "--dangerously-bypass-hook-trust" in arguments[0]
    codex_env = json.loads((tmp_path / "codex-env.jsonl").read_text())
    assert (
        codex_env["NEMO_RELAY_GATEWAY_URL"]
        == result.output["relay_runtime"]["gateway_url"]
    )


async def test_doctor_probes_codex_binary(tmp_path, restore_environ):
    ...
    assert any(
        check.name == "requirement.binary" and "codex" in check.message
...
requires_codex_binary = pytest.mark.skipif(
    ...
    reason="the codex executable is required on PATH",
)


@requires_codex_binary
async def test_real_codex_cli_against_mock_backend(api_server, tmp_path):
...
async def test_real_codex_cli_relay_semantics(api_server, tmp_path):
    config = _mock_backend_config(tmp_path, api_server)
    config.enable_relay(
        observability=RelayObservabilityConfig(
            atof=RelayAtofConfig(enabled=True, sinks=[RelayAtofFileSinkConfig()]),
...
@pytest.mark.skipif(
    os.environ.get("RUN_FABRIC_CODEX_CLI_INTEGRATION") != "1",
    reason="set RUN_FABRIC_CODEX_CLI_INTEGRATION=1 to run against the live API",
)
async def test_live_codex_cli_thread_continuity(tmp_path, restore_environ):
    restore_environ.pop("CODEX_HOME", None)
    config = fabric_config(tmp_path, mock_cli=False)
